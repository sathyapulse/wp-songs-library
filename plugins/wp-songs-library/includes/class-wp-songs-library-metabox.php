...
class Wp_Songs_Library_Metabox {
	...
	public function register_album_metaboxes( $post ) {
		add_meta_box(
			'wsl_album_attributes_metabox',
			esc_html__( 'Album attributes', 'wp-songs-library' ),
			[ $this, 'build_album_attributes_metabox' ],
			'album',
			'normal',
			'default'
		);
	...
	public function register_song_metaboxes( $post ) {
		add_meta_box(
			'wsl_song_location_metabox',
			esc_html__( 'Song location', 'wp-songs-library' ),
			[ $this, 'build_song_location_metabox' ],
			'song',
			'normal',
			'default'
		);
		...
		add_meta_box(
			'wsl_song_attributes_metabox',
			esc_html__( 'Song attributes', 'wp-songs-library' ),
			[ $this, 'build_song_attributes_metabox' ],
			'song',
			'normal',
			'default'
		);
	 ...
	 */
	public function build_album_attributes_metabox( $post, $metabox ) {
	...
	public function build_song_location_metabox( $post, $metabox ) {
		?>
		<div>
	...
	public function build_song_attributes_metabox( $post, $metabox ) {
		// Add an nonce field so we can check for it later.
	...
	public function build_year_metabox( $post ) {
		...
		if( empty( $year ) && 'song' == $post->post_type) {
			$album = get_the_terms( $post->ID, 'album' );

			if( ! empty( $album ) ) {
				$album_id = wp_list_pluck( $album, 'name', 'term_id' );
			}
		}
